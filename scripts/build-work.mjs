#!/usr/bin/env node
/**
 * Builds every shareable client project under work/ into share/work/<project>/.
 *
 ...
 * copied to share/work/<project>/ verbatim. Source files are expected to be
 ...
 * subset.
 *
 * Output gets mirrored into dist/share/work/<project>/ by scripts/copy-share.mjs
 * during astro build, so each project lands at /share/work/<project>/index.html
...
const shareWorkDir = resolve('share', 'work');
mkdirSync(shareWorkDir, { recursive: true });
...
for (const project of projects) {
  ...
  const dstDir = join(shareWorkDir, project);
  ...
  totalBytes += bytes;
  projectsBuilt++;
  console.log(`[build-work] ${project}: copied ${copied} file(s) (${fmt(bytes)}) → share/work/${project}/`);
