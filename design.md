...
**Open-behavior modifier (`--floating`).** Compose `.expanding-panel--floating` on top of either the neutral or toned variant to change how the body opens: instead of pushing siblings down (the default `<details>` reflow), the body becomes `position: absolute` below the head and floats over content beneath. The panel's grid cell stays its closed-state height, so the rest of the grid doesn't move. `z-index: 2` lifts the open panel above its siblings. Right for grids where reflow is jarring (team cards, dense card layouts); leave it off for FAQ-style stacks where in-flow growth is expected. Trade-off: a long floating body can overlap whatever's below the section, which is fine for short content but wrong for paragraph-heavy bodies — pick the modifier per use case.

**Exclusive-accordion modifier (`name=` attribute).** Give every `<details>` in the same grid the same `name="<group-slug>"` attribute to make them a mutually-exclusive accordion: opening one auto-closes the rest. Native browser behavior, no JS. Combined with `--floating`, this is the right disclosure pattern for any grid: open one, the others close, nothing else on the page moves. The group slug is just an identifier — pick one per grid; sharing a name across two unrelated grids on the same page would couple their open/close state, which is rarely what you want.

**Live references:**

- Team grid (toned, cycling six families, floating, exclusive accordion via `name="centella-team"`) — `src/pages/about-centella.astro` (`.about-team-card`)
- Styleguide showcase (mirrors the team-grid pattern via `name="sg-team"`, both variants visible) — `src/pages/styleguide.astro` (`#components` → "Expanding panel")
...
| ------------ | ---------------- | ------------------ | ---------------------------------------------------------------- |
| `violet`     | `#1E1A28`        | `#C77DFF`          | R `0.118 0.78` · G `0.102 0.49` · B `0.157 1`                    |
