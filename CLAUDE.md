...
│   ├── centella-institute.astro   # Sub-brand: hero + Centella Events section (CSS-only radio-tab
│   │                              # switch, horizontal scroller per tab) + Networks-Matter tone
│   │                              # panels + three-program editorial stack + CTA panel. Keys to
│   │                              # --networking; hero and programs-section accents use
│   │                              # display__accent--coral-orange.
...
│   └── share/
│       └── presentations/
│           └── [slug].astro  # Standalone HTML decks (e.g. ngl-barcelona, alianza-de-pais). Render at /share/presentations/<slug>/
...
└── icons/                    # Raw designer drop of icon SVGs (Title Case names with spaces, e.g. "Money Bag.svg").
                              # Not referenced at runtime — `src/assets/icons/` (kebab-case, used by the
                              # <Icon> component) is the canonical set. Public/icons/ is a staging area
                              # for icons waiting to be normalized into src/assets/icons/.
...
└── talks/                    # Speaker notes and talk prep — one <slug>.md per talk.
                              # Each file is the working script for a presentation in src/pages/share/presentations/.
                              # Cross-links to the deck file. Includes beats, timings, fill-in placeholders, nav cribs.
...
- **Events** — core content type. Has Status (Draft/Published/Archived — only Published renders on the site), Featured flag, a single `Date` property that holds either a single date or a date range (single-day events leave the end blank; multi-day events use Notion's range mode), slug, hero image, registration URL, Mailchimp tag. Page body contains description and schedule. Relations to Speakers, Attendees, Sponsors. The Notion DB also has operational-planning columns (`Event start`/`Event end` dates, `City / region`, `Host org / client`, `Event purpose`, `Success metrics`, `Audience / invitees`, `Event format`, `Working theme / narrative`, `Budget status (internal)`, `Template page`) that the website ignores — `Date` is the canonical website date.
...
- **Speakers** — Name, Title, Role, Organization, Photo, Bio. **Title** is the person's standing job title at their organization (e.g. "Strategy Director"); **Role** is their contextual function in the event (e.g. "Keynote Speaker", "Panel Moderator"). `SpeakerCard` leads with Role; Title and Organization are secondary muted lines; Bio renders below them as a short paragraph (also muted, with extra top-margin so it reads as biography rather than another tier of the title hierarchy).
- **Attendees** — Name, Title, Role, Organization, Photo. Same Title/Role split as Speakers (Role is the contextual function — "Founding Partner", "Advisor" — Title is the standing job title). No Bio column on Attendees today (asymmetric with Speakers); add it in Notion + wire through `getAttendeeById` if a future surface needs it. No `AttendeeCard` exists yet, but the Attendee data flows through and can be rendered when needed.
...
- **Don't reach for `var(--space-5)` (or any token outside the scale).** The Centella spacing scale is `1, 2, 3, 4, 6, 8, 12, 16, 24, 32` — defined in `src/styles/global.css`. CSS silently resolves unknown custom properties to nothing, so `padding: var(--space-5)` renders as zero padding without an error. Look up tokens in the scale before using them. Same rule applies to color and type tokens — the scale is the contract; if you need a value that isn't in it, add the token to the scale first or use a value from the documented set.
...
- **Don't retype a base64 data URI by paraphrase.** Mockups in `work/<project>/mockups/` embed images (e.g. the Hunt logo in the Prime Movers brochure) as `data:image/png;base64,...` so each file is fully self-contained and deploys at any path under `share/`. If you need the same image in a sibling mockup, copy the exact bytes with sed or a Python regex (`<img src="data:..." alt="...">` is a stable hook). Never reproduce the base64 payload by hand — it's opaque and the slightest character drift produces a corrupted image that still parses as valid HTML.
- **Don't try `npm run work:build` (or `share:build`) from a sandboxed shell that lacks unlink permission on the existing `share/<kind>/<project>/` tree.** The script clears the target directory before copying, which calls `rimrafSync` → `unlinkSync`; if the prior copies were written by a different process/session and the current shell can't unlink them, the build dies with `EPERM`. The host shell will run it fine. As a sandbox-side workaround for mirroring a single file change, `cp -f <work-src> <share-dst>` overwrites without unlinking and works.
- **Don't use Inter as the typeface, for anything.** Pablo's cross-project rule: Inter (including Inter Tight, Inter Variable, and any family member) is banned in body, display, fallback, and any font stack. On this codebase, the active substitute is Fira Sans (humanist sans, 100–900 weights + italics on Google Fonts) for prose/display, with Cormorant Garamond italic as the second voice. Other approved substitutions if a different tone is needed: DM Sans, Manrope, Public Sans, Space Grotesk, Geist Sans, IBM Plex Sans. System stack should be `system-ui, -apple-system, BlinkMacSystemFont, "Segoe UI", sans-serif` — never include Inter.
- **Don't use eyebrow-style headings — small uppercase letterspaced category labels above main headings.** Banned signature: small font (10–12px), `text-transform: uppercase`, wide `letter-spacing` (~0.1em+), often in mono or a contrasting weight, often with a leading rule or bullet, sitting above the actual heading as a category tag. Includes the editorial-magazine "ALL CAPS · LETTERSPACED" pattern, `.badge`-style card tags, and section eyebrows (`.stage-eyebrow`, `.rev-badge`, `.fb-label`, `.originals-label` style classes). If a section needs a label, use sentence-case body text, italic serif, or just lead with the heading directly. Folios and page numbers are fine — they're metadata, not eyebrows.
...
- **Section panels reuse one tone-coded shape.** Across the homepage pillars, Advisory's strategy panels, Institute's Networks-Matter panels, Impact's problem and solution panels, and About Centella's three pillars, every tone-coded section panel uses the same recipe: `--<family>-dark` ground + `--<family>` foreground, 8px (medium) or 16px (large/CTA) border-radius, `--space-2` outer padding, an inner frame at `1px solid color-mix(in srgb, currentColor 30%, transparent)`, and an icon chip at `color-mix(... 16%, transparent)` background + `color-mix(... 35%, transparent)` border. Title in `--color-text`, body in `--color-text-muted`, CTAs inherit `currentColor`. Each panel composition stays inside one color family (or one family + the principal violet via approved gradient on the hero accent), honoring the design.md "max two color families per composition" rule. **The next page that needs a tone panel triggers extraction into a shared `<TonePanel>` component** (currently each page duplicates the ~30 lines of CSS and ~12 lines of markup; documented in "Pending / planned work"). Until then, copy the homepage's `home-min__pillar` block as the canonical source.
- **Hero accent gradient signals page identity.** Sub-brand pages key `--page-accent` to their work-color and use a specific approved gradient on the hero accent that pairs the page key with one other family (Advisory hero → `display__accent--cyan-violet`; Institute hero + programs-section title → `display__accent--coral-orange`, networking + global; Impact hero and aspirational beat → `display__accent--lime-teal`). Umbrella pages (homepage, About Centella) don't override `--page-accent` and use `data-random-accent-gradient` on the hero accent for per-load random gradient variation. Treat this distinction as load-bearing on future pages: a sub-brand page that uses random gradient feels generic; an umbrella page locked to one gradient feels narrow.
- **CSS-only tab pattern (radio-input switch).** The Institute Events section needs a tab UI to toggle between Upcoming and Past listings, but the project's hard constraint forbids adding new client-side script bundles. Solved with two visually-hidden `<input type="radio">` siblings preceding the trigger labels and the tab panels; `:checked` + `~` sibling selectors swap which label looks active and which panel is visible. Labels carry the visible "tab" appearance and clicking them toggles the matched radio (native browser behavior). Keyboard arrow keys move between radios for free; `:focus-visible` paints a tone ring on the active label. Skipped `role="tablist"` because that ARIA contract requires JS-driven roving focus. Reach for this pattern any time a page needs a binary content switch without paying for hydration.
- **Expanding panel (click-to-reveal).** A native-`<details>`/`<summary>`-based disclosure pattern in `src/styles/global.css` (`.expanding-panel` + parts). Use whenever a panel needs a "click for more" affordance whose body is secondary content the reader can choose to open — team-member bios, FAQ answers, "more details" toggles. Browser handles open/close, focus, keyboard, and screen-reader announcement; zero JS bundle, satisfies the static-first hard constraint. The chevron is drawn from CSS borders (no icon asset). Two variants: **neutral** (default — elevated surface, right for FAQ-style disclosure) and **toned** (`.expanding-panel--toned`, right for on-brand identity panels like team cards) which applies the tone-panel visual language — family-dark ground, family-keyed border, divider, and chevron. Toned variant: consumer sets the family by overriding `background` and `color` on the panel; everything else inherits via `currentColor`. Orthogonal **`--floating`** modifier: when `[open]`, body becomes `position: absolute` below the head so siblings don't reflow (right for grids where movement is jarring; leave off for FAQ-style stacks). Floating gotcha: base `.expanding-panel` sets `overflow: hidden` to clip rounded corners on the head; that has to flip to `overflow: visible` while `[open]` or the floating body gets clipped at the panel edge (already wired in global.css). Second floating + toned gotcha: head and body each draw their own inner frame from `::before`, so when both are visible at once they have to merge — head's frame loses its bottom edge and body's frame loses its top edge (set per-side with `border-top: 0` etc. — the `border` shorthand creates cascade quirks). Markup contract + composition rules in `design.md` (Components → "Expanding panel"). Live consumers: the team grid on `/about-centella/` (`.about-team-card`, toned + cycling families + floating + exclusive accordion) and the styleguide showcase (`/styleguide#components`). When deciding between expanding-panel and `.tone-panel`: tone panels are for self-contained content blocks every reader should see; expanding panels are for content the reader chooses to open.
- **Exclusive accordion via `name=` on `<details>`.** Native HTML attribute that turns multiple sibling `<details>` into a mutually-exclusive accordion: opening one auto-closes the rest. Zero JS, satisfies the static-first rule. Applied today to the team grid (`name="centella-team"`) and the styleguide showcase (`name="sg-team"`). Combined with `.expanding-panel--floating`, this is the canonical disclosure pattern for grids: open one, the others close, nothing on the page reflows. Reach for both together on any future expanding-panel grid (speaker cards, sponsor logos). Note: the `name=` value is just a group identifier — pick a slug per grid; sharing one name across two unrelated grids on the same page would couple their open/close state, which is rarely what you want.
- **Tone-coded panels expose `--tone-dark` and `--tone` as custom properties.** When a tone-coded panel has nested elements that need to know the panel's family (the floating body's background and inner frame, for example), set `--tone-dark: var(--<family>-dark)` and `--tone: var(--<family>)` on the panel root, with the panel's own `background`/`color` referencing those properties — never hard-code the family on the panel root. Nested elements pick it up via inheritance. Without this, the floating body would inherit the page's neutral background and break the toned visual. Pattern used today on `.about-team-card--<family>` and `.sg-team-card--<family>`. Should be baked into the eventual `<TonePanel>` extraction (noted in "Pending / planned work").
- **Team grid cycles six color families.** The team cards on `/about-centella/` iterate violet → advisory → networking → investment → global → tech, wrapping at index 6 back to violet. Each card is its own composition, so the "max two color families per composition" rule still holds (the rule is per-panel, not aggregate). The styleguide showcase mirrors this for documentation. When a future surface lands a multi-card grid where each card should have its own identity color, copy the `teamFamilies` array + index-mod cycling pattern from `src/pages/about-centella.astro`.
- **Layered photo with initials fallback.** The photo-or-fallback recipe used by the team grid: a wrapper holding initials as a base layer (icon-chip recipe — `currentColor 16%` bg + `35%` border + currentColor text) plus a conditional `<img>` that overlays when a slug is present, with `onerror="this.style.display='none'"` so a 404 falls through to initials. Inline `onerror` is fine here — it's a single attribute, not a JS bundle, and there's no clean CSS-only equivalent that handles image-load failure. Live consumers: `.about-team-card__photo` and `.sg-avatar`. When a second non-team consumer ships (speaker cards, partner logos), extract `<PhotoOrInitials>` and update both call sites in the same PR.
- **Duotone photos via SVG filter.** Photos rendered inside a tone-coded panel get a duotone treatment that maps the photo's grayscale to the panel's family — shadows merge with the family-dark ground, highlights pick up the family-bright color. Real duotone via SVG `<filter>` (`feColorMatrix` for luminance grayscale + `feComponentTransfer` with two-stop `tableValues` per channel). **Critical gotcha:** every `<feFunc*>` element must carry an explicit `type="table"` attribute — without it, the function defaults to `"identity"` and `tableValues` are silently ignored, producing plain grayscale. Per-family `tableValues` table is in design.md (Photography → Duotone filter); recipe documented next to it. Since the team grid cycles six families, each consuming page now defines six `<filter>` defs (one per family — `duotone-violet`, `duotone-advisory`, `duotone-networking`, `duotone-investment`, `duotone-global`, `duotone-tech`) in an SVG defs block at the top, and a `.<card>--<family>` CSS rule maps each card class to its filter URL on the photo. SVG defs are inlined per consuming page (about-centella + styleguide showcase) — when a third surface needs duotone, promote to `Base.astro` and delete the per-page copies.
- **Team-photo pipeline.** `scripts/build-team-photos.mjs` runs *before* `astro build` (the first step in `npm run build`). For each Active Team Profile in Notion, it fetches the `Photo` file URL, runs the bytes through `sharp` (resize 800×800 cover, WebP quality 80, EXIF rotation honored), and writes to `public/team/<slug>.webp` so Astro's normal `public/` → `dist/` copy picks them up. Notion's signed file URLs expire ~1hr after issue, which is fine for the auth-gated `/tools/*` (rebuilt every deploy, staff-only) but breaks on public pages — the download-and-rewrite step is what makes the about-centella team grid stable. Output is gitignored (`public/team/` in `.gitignore`); Notion is the source of truth and every deploy regenerates. Fail-safe: per-profile errors are logged and the script exits 0, so a single missing photo doesn't block the build (the page falls back to an initials chip for that person). Run ad-hoc with `npm run team-photos:build`.
...
npm run build        # Production build: build-team-photos.mjs → astro build → build-team-assets.mjs → copy-share.mjs
...
- **Extract `<TonePanel>` component.** The tone-coded section panel pattern (homepage pillars, Advisory strategies, Institute Networks-Matter, Impact problems/solutions, About Centella three-pillars) is now duplicated across five pages. Each duplication is small (~30 lines of CSS + ~12 lines of markup) but the duplication count is high. The Institute page added the fifth — extraction is overdue. Trigger this on the very next pickup that touches any tone-panel page. At that point, extract a `<TonePanel tone="advisory|networking|investment|violet" radius="md|lg" icon="..." title="..." cta?="...">` component, fold the recipe into a single source of truth, and update all five call sites in the same PR.
